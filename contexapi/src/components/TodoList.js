...
//                 <p className="item">Go for the shopping for dinner</p>
...
const TodoList = () => {
    ...
    return (
        <div style={{ background: theme.background, color: theme.text, textAlign: 'center' }}>
            {todos.length ? (
                todos.map((todo) => {
                    return <p id={todo.id} className="item" key={todo.id} onClick={handleRemoveTodo}>{todo.text}</p>
                })
            ) : (
            ...
            <form onSubmit={handleFormSubmit}>
                ...
                <input type="text" id="todo" onChange={handleChange} value={todo}></input>
